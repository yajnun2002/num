...
class Args:
    def __init__(self):
        ...
        self.mode = "train"
        self.model_path = "./model/block12.pt" #"./model/merge/epoch_100.pt"
        self.re_times = 4
main_process = 0
args = Args()
...
with torch.no_grad():    
    gpt = GPT(args=args).cuda()
    gpt.load_state_dict(torch.load(args.model_path), strict=True)
    for param in gpt.parameters():
        param.requires_grad = False  # 冻结 GPT 层的参数
    model.gpt = gpt
